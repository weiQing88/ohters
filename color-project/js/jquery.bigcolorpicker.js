(function($){
	var bigColorPicker = new function(){
		 ...
		 */
		this.showPicker = function(callback,engine,sideLength_,_clickCallback){
			...
			$(this).data("clickCallback",_clickCallback); // 鼠标回到函数
	 ...
	 // 调用鼠标点击回调函数
	 function invokeClickCallBack(){
	 	   var callback_ = $(currentPicker).data("clickCallback");
	 	if($.isFunction(callback_)){
	 			callback_(currentColor);
	 	}	
